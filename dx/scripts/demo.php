   ...
   <table class="table table-striped" style="margin:0 auto;width:90%">
      <thead style="background-color:#E0EEEE">
        <tr>
          ...
          <th style="width:10%;">向上突破率(%)</th>
          <th style="width:10%;">向下突破率(%)</th>
      ...
      if ($dx->isok()){
        if ($dx->select_db("dx")){
            ...
            for($i=0;$i<count($dates);$i++){
                ...
                $hbr = $dx->exe_sql_one("select count(*) from dx.iknow_base where date='".$dates[$i][0]."' and hb=1");
                $lbr = $dx->exe_sql_one("select count(*) from dx.iknow_base where date='".$dates[$i][0]."' and lb=1");
                $score = $dx->exe_sql_one("select avg(scorec) from dx.iknow_attr where date='".$dates[$i][0]."'");
                $hbp = $dx->exe_sql_one("select avg(hb) from dx.iknow_tell where date='".$dates[$i][0]."'");
                $lbp = $dx->exe_sql_one("select avg(lb) from dx.iknow_tell where date='".$dates[$i][0]."'");
                $hpp = $dx->exe_sql_one("select avg(hp) from dx.iknow_tell where date='".$dates[$i][0]."'");
                $lpp = $dx->exe_sql_one("select avg(lp) from dx.iknow_tell where date='".$dates[$i][0]."'");
                echo "<tr><td>".($i+1)."</td>";
                echo "<td><a href=\"dxd.php?date=".$dates[$i][0]."\" target=\"_blank\">".$dates[$i][0]."</a></td>";
                echo "<td>".$cnt[0]."</td>";
                echo "<td>".sprintf("%.2f",100*(floatval($hbr[0])/floatval($bcnt[0])))."</td>";
                echo "<td>".sprintf("%.2f",100*(floatval($lbr[0])/floatval($bcnt[0])))."</td>";
                echo "<td>".sprintf("%.2f",100*floatval($score[0]))."</td>";
