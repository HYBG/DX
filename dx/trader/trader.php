...
class hytrader{
    ...
    public function handle_msg(){
        ...
        if (array_key_exists($param["action"],$this->msghandler)){
            ...
            $response = array("retcode"=>"10001","retmsg"=>"action type[".$param["action"]."] wrong");
        }
    ...
    ****************/
    private function rtprice($codes){
        ...
        foreach($lines as $line){
            $con = explode("\"",$line);
            $ps = explode(",",$con[1]);
            if (floatval($ps[8])>0 and ($ps[30]==$today)){
                $open = $ps[1];
                $high = $ps[4];
                $low = $ps[5];
                $close = $ps[3];
                ...
                $dt = $ps[30];
                $time = $ps[31];
                array_push(array($codes[$i],$open,$high,$low,$close,$buy,$sell,$lastclose,$dt,$time));
            }
        }
        return $hq;
    }
    ...
    private function handle_start($param){
        ...
        $sd = $this->load_sync($uid,$date);
        return $sd->serialization();
    }
    ...
    private function handle_flush($param){
        ...
        if (isset($param["list"]){
            ...
            $json = '';
            foreach($hq as $row){
                $json = $json.'{"code":'.'"'.$row[0].'","open":'.$row[1].',"high":'.$row[2].',"low":'.$row[3].',"close":'.$row[4].',"buy":'.$row[5].',"sell":'.$row[6].',"lastclose":'.$row[7].',"date":'.$row[8].',"time":'.$row[9].'},';
            }
            if (strlen($json)>0){
                $json = substr($json,0,-1);
                $json = '{"retcode":0,"hq":['.$json.']}';
            }
            else{
                $json = traderreturncode::to_josn(traderreturncode::SUCCESS);
            }
        }
        else{
            $json = traderreturncode::to_josn(traderreturncode::LIST_IS_NOT_SET);
        }
        return $json;
    }
