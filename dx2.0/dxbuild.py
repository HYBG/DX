...
class attrdata:
    ...
    def fv(self):
        return '%d%d%d%d%d'%(self.hb,self.lb,self.k,self.v1,self.v2)

    def linar(self):
        return '%s,%s,%s,%0.4f,%0.4f,%0.4f,%0.4f'%(self.code,self.date,self.fv(),self.stdzdf,self.stdzf,self.stdosrc,self.stdcsrc)
        
    def connect(self):
        return '%s,%s,%s,%0.4f,%0.4f,%0.4f,%0.4f,%d,%d,%d,%d,%d,%d,%d,%0.4f,%0.4f,%0.4f,%0.4f'%(self.code,self.date,self.fv(),self.stdzdf,self.stdzf,self.stdosrc,self.stdcsrc,self.wonder.hb,self.wonder.lb,self.wonder.k,self.wonder.v1,self.wonder.v2,self.wonder.hp,self.wonder.lp,self.wonder.openzdf,self.wonder.highzdf,self.wonder.lowzdf,self.wonder.closezdf)
...
class iksetup:
    ...
    def _basewonder(self,cur,last):
        openzdf = (cur.open-last.close)/last.close
        highzdf = (cur.high-last.close)/last.close
        lowzdf = (cur.low-last.close)/last.close
        closezdf = (cur.close-last.close)/last.close
        return self._basek(cur,last)+(openzdf,highzdf,lowzdf,closezdf)

    def _baseattr(self,cur,last,before):
        vol = (cur.close-cur.open)*cur.volh
        yvol = (last.close-last.open)*last.volh
        bvol = (before.close-before.open)*before.volh
        v1 = 0
        if (vol-yvol)>0:
            v1 = 1
        v2 = 0
        if (vol+bvol-2*yvol)>0:
            v2 = 1
        return self._basek(cur,last)+(v1,v2)
    ...
    def _genattr(self,need):
        info = self._baseattr(need[-1],need[-2],need[-3])
    ...
    def _genwonder(self,current,prev,next,before):
        wonder = wonderdata()
        poles = self._poles(current,prev,next)
        base = self._baseattr(current,prev,before)
        bw = self._basewonder(current,prev)
    ...
    def setup(self):
        g_logger.info('dx setup handle start....')
        ...
        for fn in files:
            ...
            print 'dx setup deal with file[%s]'%ffn
            ...
            for i in range(self._minusefullength,len(mat)-2):
                ...
                wonder = self._genwonder(mat[i+1],mat[i],mat[i+2],mat[i-1])
                if wonder.lowzdf<=-0.101:
                    continue
                attr.wonder = wonder
                amt.append(attr)
    ...
    def importdb(self):
        basedir = self._basedir
        self._inputdir(basedir,'dx','iknow_base')
