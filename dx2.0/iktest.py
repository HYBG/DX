...
class iktest:
    def __init__(self):
        ...
        self._logger = g_iu.createlogger('iktest',os.path.join(os.path.join(g_home,'log'),'iktest.log'),logging.INFO)
            ...
            if len(blanks)>0:
                ...
                for dt in blanks:
                    if kinfo.has_key(dt) and kd.has_key(dt) and boll.has_key(dt) and macd.has_key(dt) and ma.has_key(dt) and vol.has_key(dt):
                        sqls.append(('insert into hy.iknow_attr(code,date,hb,lb,kline,csrc,zdf,k,d,mid,std,diff,dea,macd,ma5,ma10,ma30,ma60,vol3,vol5,vol10,vol20) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)',(code,dt,)+kinfo[dt]+kd[dt]+boll[dt]+macd[dt]+ma[dt]+vol[dt]))
                g_tool.task(sqls)
    ...
    def feature(self):
        self._logger.info('iktest feature task start....')
        codes = self._codes()
        total = float(len(codes))
        handled = 0
        for code in codes:
            sqls = []
            handled = handled+1
            data = g_tool.exesqlbatch('select date,hb,lb,kline,csrc from hy.iknow_attr where code=%s order by date desc',(code,))
            if len(data)<200:
                continue
            if data[199][0]<'2005-01-01':
                continue
            ld = g_tool.exesqlone('select date from test.iknow_feature where code=%s order by date desc limit 1',(code,))
            start = '2005-01-01'
            ...
            for i in range(len(data)-4):
                ...
                fv = '%s%d'%(fv,self._qsrc(data[i+1][4]))
                ffv = '%s%d'%(ffv,self._qsrc(data[i][4]))
                ...
                if len(ohlc)==2:
                    ...
                    sqls.append(('insert into test.iknow_feature(code,date,fv,openr,highr,lowr,closer,ffv,next) values(%s,%s,%s,%s,%s,%s,%s,%s,%s)',(code,data[i][0],fv,openr,highr,lowr,closer,ffv,next)))
            g_tool.task(sqls)
            self._logger.info('iktest feature handle progress[%0.2f%%] code[%s] sqls[%d]....'%(100*(handled/total),code,len(sqls)))
        self._logger.info('iktest feature task done....')
    ...
    def prob(self,first=False):
        self._logger.info('iktest prob task start....')
        fvs = g_tool.exesqlbatch('select distinct ffv from test.iknow_feature',None)
        sqls = []
        for fv in fvs:
            self._logger.info('iktest prob deal with fv[%s]....'%fv[0])
            cnt = g_tool.exesqlone('select count(*) from test.iknow_feature where fv=%s',(fv[0],))
            mat = []
            if cnt[0]!=0:
                for i in range(1,9):
                    data = g_tool.exesqlbatch('select openr,highr,lowr,closer from test.iknow_feature where fv=%s and next=%s',(fv[0],i))
    ...
    def classify(self):
        ...
        for dt in dts:
            ...
            for tg in tags:
                ...
                vol3 = g_tool.exesqlone('select sum(vol3) from hy.iknow_attr where date=%s and code in (select code from hy.iknow_tags where tag=%s)',(dt[0],tag))
                vol5 = g_tool.exesqlone('select sum(vol3) from hy.iknow_attr where date=%s and code in (select code from hy.iknow_tags where tag=%s)',(dt[0],tag))
                vol10 = g_tool.exesqlone('select sum(vol3) from hy.iknow_attr where date=%s and code in (select code from hy.iknow_tags where tag=%s)',(dt[0],tag))
                vol20 = g_tool.exesqlone('select sum(vol3) from hy.iknow_attr where date=%s and code in (select code from hy.iknow_tags where tag=%s)',(dt[0],tag))
                if cnt[0]!=0:
                    hbr = round(float(hbs[0])/float(cnt[0]),4)
                    lbr = round(float(lbs[0])/float(cnt[0]),4)
                    csrc = round(csrc[0],4)
                    zdf = round(zdf[0],4)
                    vol3ev = round(vol3[0]/3.0,4)
                    vol5ev = round(vol5[0]/5.0,4)
                    vol10ev = round(vol10[0]/10.0,4)
                    vol20ev = round(vol20[0]/20.0,4)
                    sqls.append(('insert into hy.iknow_classify(name,date,type,count,hbr,lbr,csrc,zdfev,vol3ev,vol5ev,vol10ev,vol20ev) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)',(tag,dt[0],typ,cnt[0],hbr,lbr,csrc,zdf,vol3ev,vol5ev,vol10ev,vol20ev)))
            g_tool.task(sqls)
