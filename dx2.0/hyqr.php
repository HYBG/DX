...
<?php
    ...
    echo "<div>";
    if (count($prev)>0){
        echo "<h2><a href=\"hyqr.php?date=".$prev[0]."\">prev</a></h2>";
    }
    echo "<h2 id=\"H2\"></h2>";
    if (count($next)>0){
        echo "<h2><a href=\"hyqr.php?date=".$next[0]."\">next</a></h2>";
    }
    echo "</div>";
?>
...
<?php
    ...
    foreach($codes as $code){
        $data = $dx->exe_sql_batch("select k,d,mid,std from hy.iknow_bollkd where code='".$code[0]."' and date<='".$date."' order by date desc limit 3");
        if (count($data)<3){
            continue;
        }
        if ($data[0][0]>$data[0][1] and $data[0][1]>55){
            $base = $dx->exe_sql_batch("select close,volwy from hy.iknow_data where code='".$code[0]."' and date<='".$date."' order by date desc limit 3");
            $close = $base[0][0];
            $volwy = $base[0][1];
            $yc = $base[1][0];
            $byc = $base[2][0];
            ...
            $bup = $data[2][2]+2*$data[2][3];
            if ($close>$up and $yc>$yup and $byc<$bup){
